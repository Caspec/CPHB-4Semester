...
public class grid : MonoBehaviour {
    ...
    void InitCells()
    {
        ...
        //random colors gameObject in array
        ...
        for (int row = 0; row < rows; row++)
        {
            for (int col = 0; col < cols; col++)
            {
              ...
              //  print("x: " + pos.x);
              //  print("y: " + pos.y);
              //  0.25, 0.25 - + 0.50 , if 10 rows x 10 cols
                
                GameObject cO = Instantiate(cellObject, pos, Quaternion.identity) as GameObject;
                cO.GetComponent<Renderer>().material.color = colors[Random.Range(0, colors.Length)];

                //if(pos.y == -2.25 || pos.x == -2.25)
                //{

                    //GameObject c1 = Instantiate(cellObject, pos, Quaternion.identity) as GameObject;

                    //print("Good job pos.y -2.25 " + pos);
                    //instantiate the game object, at position pos, with rotation set to identity
                    //GameObject cO = Instantiate(cellObject, pos, Quaternion.identity) as GameObject;

                    //color gameObject
                    //cO.GetComponent<Renderer>().material.color = Color.green;
                //}
